...
async function fetchShareData(tradeId: string): Promise<ShareData | null> {
  try {
    ...
    if (tradeRaw.visibility === "private") return null;
    ...
    return {
      trade: {
        pair: tradeRaw.pair as string,
        direction: tradeRaw.direction as TradeData["direction"],
        entryDate: tradeRaw.entryDate as string,
        exitDate: tradeRaw.exitDate as string,
        rr: (tradeRaw.rr as number) ?? 0,
        result: (tradeRaw.result as number) ?? 0,
        netPnl: (tradeRaw.netPnl as number) ?? 0,
        strategy: (tradeRaw.strategy as string) ?? "",
        note: (tradeRaw.note as string) ?? "",
        screenshotUrl: (tradeRaw.screenshotUrl as string) ?? "",
      },
      user: {
